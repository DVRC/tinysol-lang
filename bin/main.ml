...
(*~
 * Explanation:
 * First parse the arguments. Can have either 2 or 3 commands
 *)
...
| 3 when Sys.argv.(1) = "exec_cmd" -> (match read_line() with
    | Some s when s <> "" -> s
      |> parse_cmd |> blockify_cmd (* TODO: enumify? *)
...
| 2 when Sys.argv.(1) = "parse" -> (match read_line() with
    | Some s when s<>"" -> s |> parse_contract |> preprocess_contract
      |> string_of_contract |> print_string
    | _ -> print_newline())
| 3 when Sys.argv.(1) = "parse" -> (match read_file Sys.argv.(2) with
      "" -> print_newline()
    | s -> s |> parse_contract |> preprocess_contract
      |> string_of_contract |> print_string)

(* typecheck contract*)
| 2 when Sys.argv.(1) = "typecheck" -> (match read_line() with
    | Some s when s<>"" -> s
      |> parse_contract |> preprocess_contract |> typecheck_contract
