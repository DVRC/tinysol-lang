...
(*~ List.mem x list: true iff x is a member of the set.
 * In this case, it's used to discard duplicates.
 * The @ operator appends a list to the front of the list
 *)
let rec union l1 l2 = match l1 with
    [] -> l2 (*~ Just return l2 *)
  | car::cdr -> (if List.mem car l2 then [] else [car]) @ union cdr l2
...
let rec vars_of_expr = function
  ...
  | This
  | BlockNum
  | EnumOpt _ -> [] (*~ No variable, just an empty list *)
  | Var x -> [x]
  | BalanceOf e
  | Not e
  | IntCast e
  ...
  | FunCall(e_to, _ , e_value, e_args) ->
    union (vars_of_expr e_to)
      (union (vars_of_expr e_value)
         (List.fold_left
            (fun acc ea -> union acc (vars_of_expr ea)) [] e_args)) (*~ What is ea? *)
...
and vars_of_cmd = function
  ...
  | ProcCall(e_to, _, e_value, e_args) ->
    union (vars_of_expr e_to)
      (union (vars_of_expr e_value)
         (List.fold_left (fun acc ea -> union acc (vars_of_expr ea)) [] e_args))

(*~ Takes contracts and returns a list of identifiers (names) *)
...
let string_of_exprval_list el =
  let rec helper el = match el with
    | [] -> ""
    | e::rest -> "," ^ string_of_exprval e ^ helper rest
  in match el with
  | [] -> ""
  | e::rest -> "(" ^ string_of_exprval e ^ helper rest ^ ")"
...
let string_of_args =
  List.fold_left (fun s a -> s ^ (if s <> "" then "," else "") ^ (string_of_exprval a)) ""
...
let rec string_of_expr = function
  ...
  | FunCall(e_to, f, e_value, e_args) ->
    string_of_expr e_to ^ "." ^ f ^ "{value:" ^
    string_of_expr e_value ^ "}" ^ "(" ^
    (List.fold_left (fun acc ea -> acc ^ (if acc="" then "" else ",") ^ string_of_expr ea) "" e_args) ^ ")"
  | ExecFunCall(c) -> "<" ^ string_of_cmd c ^ ">"

(*~ el' renamed to rest *)
and string_of_expr_list el =
  let rec helper el = match el with
    | [] -> ""
    | e::rest -> "," ^ string_of_expr e ^ helper rest
  in match el with
  | [] -> ""
  | e::rest -> "(" ^ string_of_expr e ^ helper rest ^ ")"
...
and string_of_cmd = function
  ...
  | Decons(_) -> failwith "TODO: multiple return values"
  | MapW(x, ek, ev) ->
    x ^ "[" ^ string_of_expr ek ^ "] = " ^ string_of_expr ev ^ ";"
...
and string_of_var_type = function
  ...
  | MapT(tk,tv) ->
    "mapping (" ^ string_of_base_type tk ^ " => " ^ string_of_base_type tv ^ ")"
...
and string_of_var_decl (vd: var_decl) : string =
  ...
  vd.name ^ (string_of_init_value vd.init_value)

and string_of_local_var_decl (vd: local_var_decl): string =
  string_of_var_type vd.ty ^ " " ^ vd.name

let string_of_var_decls =
  List.fold_left
    (fun str d -> str ^ (if str <> "" then ";\n  " else "  ") ^ string_of_var_decl d) ""

let string_of_local_var_decls =
  List.fold_left
    (fun str d ->
       str ^ (if str <> "" then ";\n  " else "  ") ^
       string_of_local_var_decl d
    ) ""

let string_of_fun_args =
  List.fold_left
    (fun str d ->
