...
type expr =
  ...
  | AddrConst of addr
  | BlockNum		(*~ IDK what's this *)
  | This		(*~ Reflection? *)
  | Var of ide
  | MapR of expr * expr
  | BalanceOf of expr	(*~ I assume it's an operator *)
  | Not of expr
  | And of expr * expr
  | Or of expr * expr
  | Add of expr * expr
...
and cmd =
  ...
  | Send of expr * expr			(* send(e1, e2) transfers e2 wei to e1 *) (*~ Wei is smallest subunit of ether *)
...
(* a variable declaration (t, x) consists of:
   - a type t
   - an identifier x of the variable (*~ x is a name!  *)
   - a variable visibility modifier (default is internal)
   - a variable mutability modifier (default is mutable)
 *)
...
(* Transactions contain:
 - txsender: the address of the transaction sender (either an EOA or a
   contract address)
 - txto: the address of the called contract
 - txfun: the name of the called function. In a deploy transaction, the txfun is
   "constructor" and the first argument is the contract code
 - txvars: the list of actual parameters
 - txvalue: the amount of wei transferred from the sender to the caller along
