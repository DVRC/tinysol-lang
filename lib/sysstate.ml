...
let pop_callstack (state: sysstate): sysstate = match state.callstack with
    [] -> failwith "empty call stack"
  | _::fl -> { state with callstack = fl } (* returns state with callstack[1:]*)

let push_callstack (fr: frame) (state: sysstate): sysstate =
  { state with callstack = fr :: state.callstack }

let pop_locals (state: sysstate): sysstate = match state.callstack with
    [] -> failwith "empty call stack"
  | frame::fl -> match frame.locals with
    | [] -> failwith "empty locals stack"
    | _::envlist -> let frame' = {frame with locals = envlist } in
      { state with callstack = frame'::fl }

(* initial (empty) environment *) (*~ bottom state *)
...
let lookup_var (x: ide) (st: sysstate): exprval option =
  ...
  | None ->
    ...
    try Some (cs.storage x) with _ -> None
...
let lookup_balance (a: addr) (st: sysstate): int =
  try (st.accounts a).balance with _ -> 0
